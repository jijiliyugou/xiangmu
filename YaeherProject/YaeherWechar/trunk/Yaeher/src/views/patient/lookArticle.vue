<template>
    <div class="article-look padding-top">
		<mt-header fixed title="看文章">
        </mt-header>
        <div ref="srcollBox" class="content listAbout">
            ...
            <ul v-infinite-scroll="getArticleList"
				...
				<li v-for="(item, index) in articleList" :key="index" class="">
			        <router-link :to="{path: '/article-detail', query: {id: item.id, look: 'no'}}" class="articleList">
...
export default {
	...
	methods: {
		getArticleList (flag) {
            ...
            this.instance.releaseManagePage({
				keyWord,
				checkState: 'success',
				skipCount,
				maxResultCount
