...
export default {
  ...
  methods: {
    update() {
      ...
      this.updateDataInfo(this.$api.labelgroupUpdate,this.dataInfo,"list");
    },
    ...
    setDefaultCheck() {
      //默认选中
      this.$http.post(this.$api.labelgroupData,{id:this.$route.query.id}).then(res=>{
        if(res.result.code==200){
          this.dataInfo=res.result.item;
          this.checkedRole=this.dataInfo.lableID.split(",").map(function(data){return +data;});        
        }
      }).catch(error=>{});
    }
  },
