<template>
  <section class="yh-body" v-loading="loading">
    ...
    <el-form  ref="form" :model="dataInfo"
      ...
      <el-form-item v-if="roleType('CustomerService')==true" label="联系电话：" class="flex-50">
      ...
      <template v-if="dataInfo.consultState=='已退单'">
        <!-- <el-form-item label="退单理由：" class="flex-100">
            <span class="itemval" v-text="(dataInfo.refundReason).LabelName"></span>
        </el-form-item> -->
      ...
      <el-form-item label="质控评分：" class="flex-100" v-if="dataInfo.isQuality">
          <span class="itemval" v-text="dataInfo.qualityLevel"></span>
      </el-form-item>
      <el-form-item label="质控评语：" class="flex-100"  v-if="dataInfo.isQuality">
    ...
    <div class="form-footer">
        <el-button type="success"  @click="$router.go(-1)"
           >返回</el-button>
        <el-button v-if="roleType('CustomerService')==true" type="primary"  @click="routerLink('/message/list','list',dataInfo.consultantID)"
           >联系用户</el-button>
        <template v-if="roleType('CustomerService')!=true">
          <el-button v-if="dataInfo.hasCollect==false" type="info"  @click="collect('collect')"
             >收藏</el-button>
          <el-button type="info" @click="collect('cancel')" v-else
             >取消收藏</el-button>
        </template>
    </div>
    <!--图片查看视频播放-->
...
export default {
  ...
  methods:{
    ...
    collect(type){
      this.$http.post(this.$api.collect,{doctorID:this.dataInfo.doctorID,consultID:this.$route.query.id}).then(res=>{
        if(res.result.code==200){
          if(type=='collect'){
            this.dataInfo.hasCollect=true;
            this.successMessage("收藏成功");
          }else{
            this.dataInfo.hasCollect=false;
