<template>
  <section class="yh-body" v-loading="loading">
    <!-- 返回组件 -->
    <yh-back></yh-back> 
    <el-form  ref="form" :model="dataInfo"
        class="yh-form" label-width="150px">
      <el-form-item label="订单单号：" class="flex-100">
          <span class="itemval" v-text="dataInfo.orderNumber"></span>
      </el-form-item>
      <el-form-item label="咨询单号：" class="flex-100">
          <span class="itemval" v-text="dataInfo.consultNumber"></span>
      </el-form-item>
      <el-form-item label="咨询姓名：" class="flex-100">
          <span class="itemval" v-text="dataInfo.consultantName"></span>
      </el-form-item>
      <el-form-item label="患者姓名：" class="flex-100">
          <span class="itemval" v-text="dataInfo.patientName"></span>
      </el-form-item>
      <el-form-item label="医生姓名：" class="flex-100">
          <span class="itemval" v-text="dataInfo.doctorName"></span>
      </el-form-item>
      <el-form-item label="咨询类型：" class="flex-100">
          <span class="itemval" v-text="typeCheck(dataInfo.consultType)"></span>
      </el-form-item>
      <!-- <el-form-item label="疾病类型：" class="flex-100">
          <span class="itemval" v-text="dataInfo.iiInessType"></span>
      </el-form-item> -->
      <el-form-item label="订单币别：" class="flex-100">
          <span class="itemval" v-text="dataInfo.orderCurrency"></span>
      </el-form-item>
      <el-form-item label="订单总金额：" class="flex-100">
          <span class="itemval" v-text="dataInfo.orderMoney"></span>
      </el-form-item>
      <el-form-item label="新增时间：" class="flex-100">
          <span class="itemval" v-text="formatTime(dataInfo.createdOn)"></span>
      </el-form-item>
    </el-form>       
    <div class="form-footer">
        <el-button type="success"  @click="$router.go(-1)"
           >返回</el-button>
    </div>           
</section>
</template>
<script>
import formMixns from "@/mixins/form";
export default {
  mixins: [formMixns],
  data() {
    return {
      dataInfo:{
      }
    }
  },
  methods:{
    typeCheck(type){
      if(type=='Phone'){return '电话'}else if(type=='ImageText'){return '图文'}
    }
  },
  created(){
    this.getDataInfo(this.$api.orderDetail,{id:this.$route.query.id});
  }
};
</script>

<style scoped>
.yh-form .el-form-item{padding:0px;}
.itemval{display: inline-block;max-width: 500px;line-height: 20px;margin-top: 6px;}
</style>