  ...
  <section v-loading="loading" class="yh-body">
        <div class="yh-list-header flex-between">
            <el-button type="primary" 
             icon="el-icon-plus"
              @click="routerLink('/fastreply/add','add',0)"
              >添加</el-button>
            <!-- 右操作 -->
             <div>
        ...
        <div class="yh-container">
           <el-table :data="tableData" border style="width: 100%">
                ...
                <el-table-column label="操作" align="center">
                  <div slot-scope="scope">
                      <yh-button type="edit" @click.native="routerLink('/fastreply/update','update',scope.row.id)"
                      ></yh-button>
                       <yh-button type="delete" @click.native="deleteBatch($api.fastreplyDelete,scope.row.id)"
                       ></yh-button>
                  </div>
                 </el-table-column>
           </el-table>
...
export default {
  ...
  activated () {
    ...
    this.initTableData(this.$api.fastreplyList,this.params);
  },
