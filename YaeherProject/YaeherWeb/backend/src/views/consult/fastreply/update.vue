...
export default {
  ...
  methods: {
    ...
    update() {
      this.updateDataInfo(this.$api.fastreplyUpdate,this.dataInfo,"list");
    }
  },
  created(){
    this.$http.post(this.$api.fastreplyData,{id:this.id})
      .then(res => {  
          if (res.result.code == "200") {
            this.dataInfo=res.result.item;
            this.letterNum=res.result.item.content.length;
          }
          this.loading = false;
