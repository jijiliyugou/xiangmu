<template>
  <section class="yh-body" v-loading="loading">
    ...
    <el-form  ref="form" :model="dataInfo" :rules="rules" 
      ...
      <el-form-item v-if="dataInfo.paperFrom=='doctor'" label="医生姓名：" class="flex-100">
      ...
      <el-form-item v-if="dataInfo.consultNumber!=null" label="案例地址" class="flex-100">
...
export default {
  ...
  methods: {
    update() {
      ...
      this.updateDataInfo(this.$api.articleUpdate,this.dataInfo,"list");
    },
    getUeditor(ue,index){
  ...
  created(){
    ...
    axios.post(this.$api.articleData,{id:this.$route.query.id})
        .then(res => {  
