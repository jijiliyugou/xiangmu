  ...
  <section v-loading="loading" class="yh-body">
        <div class="yh-list-header flex-between">
            <el-button type="primary" 
             icon="el-icon-plus"
              @click="routerLink('/doctorarticle/add','add',0)"
              >添加</el-button>
            <!-- 右操作 -->
             <div>
                ...
                <el-select placeholder="文章状态" v-model="params.checkState" @change="query">
                     ...
                     <el-option label="已推送" :value="'Push'"></el-option>
        ...
        <div class="yh-container">
           <el-table :data="tableData" border style="width: 100%">
                ...
                <el-table-column prop="paperFrom" label="文章来源" align="center">
                  <div slot-scope="scope">
                    {{scope.row.paperFrom=='person'?'个人':'案例'}} 
                  </div>
                </el-table-column>
                ...
                <el-table-column label="操作" align="center">
                  <div slot-scope="scope">
                      <yh-button v-if="scope.row.checkState=='created'" type="send" @click.native="send(scope.row.id)"
                      ></yh-button>
                      <yh-button type="edit" @click.native="routerLink('/doctorarticle/update','update',scope.row.id)"
                      ></yh-button>
                      <yh-button type="delete" @click.native="deleteBatch($api.doctorarticleDelete,scope.row.id)"
                       ></yh-button>
                  </div>
                 </el-table-column>
           </el-table>
...
export default {
  ...
  methods:{
    ...
    send(id){
      this.$confirm('是否确定推送文章？', '警告', { type: "error" })
      .then(mes => {
          this.$http.post(this.$api.doctorarticleSend,{id:id}).then(res=>{
            if(res.result.code==200){
              this.successMessage("文章推送成功");
              this.initTableData(this.$api.doctorarticleList,this.params);
            }
          }).catch(error=>{this.errorMessage("文章推送失败");});
      })
      .catch(error => { });
