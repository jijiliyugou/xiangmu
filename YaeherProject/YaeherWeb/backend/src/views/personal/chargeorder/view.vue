<template>
  <section class="yh-body" v-loading="loading">
    ...
    <el-form  ref="form" :model="dataInfo"
      ...
      <el-form-item v-if="dataInfo.qualityLevel!=0" label="我的评分：" class="flex-100">
          <span class="itemval" v-text="dataInfo.qualityLevel"></span>
      </el-form-item>
      <el-form-item v-if="dataInfo.repayIllnessDescription!=null" label="评论描述：" class="flex-100">
          <span class="itemval" v-text="dataInfo.repayIllnessDescription"></span>
      </el-form-item>
      <el-form-item label="处理状态：" class="flex-100">
          <span class="itemval" v-text="typeState(dataInfo.replyState)"></span>
      </el-form-item>
      <el-form-item v-if="isType('reply')" label="您的评分：" class="flex-100">
      ...
      <el-form-item v-if="isType('reply')" style="margin-top:10px;" label="评价描述" class="flex-100" prop="reson">
      <div class="reasonbox"> 
        <el-input type="textarea" :rows="6" placeholder="请输入您的评价描述" v-model="reson" class="yh-width-fixed"></el-input>
      </div>
    </el-form-item>
    </el-form>       
    <div class="form-footer" v-if="isType('reply')">
        <el-button type="primary"  @click="gradeget()"
           >确定</el-button>
...
export default {
  ...
  methods:{
    ...
    gradeget(){
      ...
      if(this.reson.trim()==""){
        this.errorMessage("评价描述不能为空");
        return false;
      }
      this.handleDataInfo(this.$api.chargedoctorput,{id:this.dataInfo.id,qualityLevel:this.grade,repayIllnessDescription:this.reson,replyState: 'treated'},"/chargeorder/list");
    },
    back(){
      this.handleDataInfo(this.$api.chargedoctorput,{id:this.dataInfo.id,replyState: 'back'},"/chargeorder/list");
    },
    typeCheck(type){
  ...
  created(){
    this.$http.post(this.$api.chargedoctororderview,{id:this.$route.query.id}).then(res=>{
