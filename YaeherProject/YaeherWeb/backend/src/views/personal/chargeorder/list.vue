<template>
  <section v-loading="loading" class="yh-body">
        <div class="yh-list-header flex-between">
             <div>
                <label class="yh-label">处理状态：</label>
                <el-select placeholder="处理状态" v-model="params.RefundState" @change="query">
                     <el-option label="全部" :value="''"></el-option>
                     <el-option label="未处理" :value="'untreated'"></el-option>
                     <el-option label="已处理" :value="'treated'"></el-option>
                     <el-option label="已退回" :value="'return'"></el-option>
                </el-select>
           ...
           <el-table :data="tableData" border style="width: 100%">
                ...
                <!-- <el-table-column label="头像" width="80" align="center">
                  <div slot-scope="scope">
                     <img :src="scope.row.doctorImage==null?'./../static/images/userlogo.png':scope.row.doctorImage" class="userimages">
                  </div>
                </el-table-column> -->
                ...
                <!-- <el-table-column prop="qualityLevel" label="质控评分" align="center">
                </el-table-column>
                <el-table-column prop="qualityReason" label="质控评语" align="center">
                  <div slot-scope="scope" class="wrapclass">
                    {{scope.row.qualityReason}} 
                  </div>
                </el-table-column> -->
                <el-table-column prop="qualityControlManageState" label="处理状态" align="center"></el-table-column>
                ...
                <el-table-column label="操作" align="center">
                  <div slot-scope="scope">
                      <yh-button v-if="scope.row.qualityControlManageState=='未处理'&&roleType('QC')!=true" type="reply" @click.native="routerLink('/chargeorder/view','reply',scope.row.id)"
                      ></yh-button>
                      <yh-button type="view" @click.native="routerLink('/chargeorder/view','view',scope.row.id)"
                      ></yh-button>
                  </div>
                 </el-table-column>
           </el-table>
...
export default {
  ...
  activated () {
    ...
    this.initTableData(this.$api.chargedoctororderList,this.params);
  },
