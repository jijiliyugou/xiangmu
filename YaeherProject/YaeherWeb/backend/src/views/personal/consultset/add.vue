...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {//校验规则
        serviceExpense: [required,number],
  ...
  methods: {
    update() {
      this.saveDataInfo(this.$api.consultsetAdd,this.dataInfo,"list");
    }
  },
  created(){
    this.loading=false;
    this.dataInfo.serviceTypeValue=this.id;
    this.dataInfo.serviceType=this.type;
  }
};
</script>
