  ...
  <section class="yh-body" v-loading="loading">
    ...
    <el-form  ref="form" :model="dataInfo" :rules="rules" 
      ...
      <el-form-item label="用户角色" class="flex-100" prop="roleCode">
          <el-select placeholder="请选择用户角色" v-model="dataInfo.roleCode" @change="getName" :disabled="true">
...
export default {
  ...
  methods: {
    update() {
      ...
      this.updateDataInfo(this.$api.wechatUpdate,this.dataInfo,"list");
    },
    getName(val){
  ...
  created(){
    axios
        .all([
          ...
          axios.post(this.$api.wechatData,{id:this.id}), //默认数据 
        ])
        .then(
