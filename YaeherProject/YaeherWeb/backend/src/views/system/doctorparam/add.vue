  ...
  <section class="yh-body" v-loading="loading">
    ...
    <el-form  ref="form" :model="dataInfo" :rules="rules" 
      ...
      <el-form-item label="代码编号" class="flex-100" prop="doctorParaSetCode">
          <el-input v-model="dataInfo.doctorParaSetCode" class="yh-width-fixed"></el-input>
      </el-form-item>
      <el-form-item label="名称" class="flex-100" prop="doctorParaSetName">
          <el-input v-model="dataInfo.doctorParaSetName" class="yh-width-fixed"></el-input>
      </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      rules: {//校验规则
        doctorParaSetCode: [required],
        doctorParaSetName: [required],
        itemValue: [required]
      }
    };
  },
  methods: {
    update() {
      this.saveDataInfo(this.$api.doctorparamAdd,this.dataInfo,"list");
    }
  },
  created(){
    this.loading=false;
