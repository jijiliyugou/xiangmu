  ...
  <section class="yh-body" v-loading="loading">
    ...
    <el-form  ref="form" :model="dataInfo" :rules="rules" 
      ...
      <el-form-item label="标签类型" class="flex-100" prop="labelTypeCode">
          <el-select placeholder="请选择类型" v-model="dataInfo.labelTypeCode" @change="getName" :disabled="this.id!=0">
...
export default {
  ...
  methods: {
    update() {
      this.saveDataInfo(this.$api.labelstartAdd,this.dataInfo,"list");
    },
    getName(val){
    ...
    axios
        ...
        .then(
          axios.spread((data) => {
            ...
            if(this.type!='add'){
              this.dataInfo.labelTypeCode=this.type.split(",")[0];
              this.dataInfo.labelTypeName=this.type.split(",")[1];
            }
            this.loading = false;
          })
        )
