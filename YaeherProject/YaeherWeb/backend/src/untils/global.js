...
export default {
    install(Vue, options) {
        ...
        Vue.prototype.formatTime = function (value) {
            ...
            let result=time.getFullYear() + '-' + ((time.getMonth() + 1)<10?'0'+(time.getMonth() + 1) : (time.getMonth() + 1))+ '-' + (time.getDate()<10?'0'+time.getDate():time.getDate()) + ' ' + (time.getHours()<10?'0'+time.getHours():time.getHours()) + ':' + (time.getMinutes()<10?'0'+time.getMinutes():time.getMinutes()) + ':' + (time.getSeconds()<10?'0'+time.getSeconds():time.getSeconds());
            return result;
        }
        //日期格式化全局方法
        Vue.prototype.formatDate = function (value) {
            let time= new Date(value);
            let result=time.getFullYear() + '-' + ((time.getMonth() + 1)<10?'0'+(time.getMonth() + 1) : (time.getMonth() + 1))+ '-' + (time.getDate()<10?'0'+time.getDate():time.getDate());
            return result;
        }
        //时间格式化全局方法
        Vue.prototype.secondTime = function (value) {
            let time= new Date(value);
            let result=time.getFullYear() + '-' + ((time.getMonth() + 1)<10?'0'+(time.getMonth() + 1) : (time.getMonth() + 1))+ '-' + (time.getDate()<10?'0'+time.getDate():time.getDate()) + ' ' + (time.getHours()<10?'0'+time.getHours():time.getHours()) + ':' + (time.getMinutes()<10?'0'+time.getMinutes():time.getMinutes()) + ':' + (time.getSeconds()<10?'0'+time.getSeconds():time.getSeconds());
            return result;
        }
        //上传图片命名
        Vue.prototype.uploadName = function () {
            let result='';//六位随机数
            let time=new Date();
            for(var i=0;i<6;i++){
                result+=Math.floor(Math.random()*10);
            }
            result='PC'+time.getFullYear() + ((time.getMonth() + 1)<10?'0'+(time.getMonth() + 1) : (time.getMonth() + 1)) + (time.getDate()<10?'0'+time.getDate():time.getDate()) + (time.getHours()<10?'0'+time.getHours():time.getHours()) + (time.getMinutes()<10?'0'+time.getMinutes():time.getMinutes()) + (time.getSeconds()<10?'0'+time.getSeconds():time.getSeconds())+result;//时间戳+六位随机数
            return result;
        }
        //角色判断函数
        Vue.prototype.roleType = function (role) {
            let resultList=JSON.parse(localStorage.getItem('role'));
            for(var i=0;i<resultList.length;i++){
                if(role==resultList[i]){
                    return true;
                }
            }
        }
