...
(function () {
    ...
    function loginUserInternal(tenantId, callback) {
		...
		if (!secret) {
            alert('secret is required, please try with a valid value !');
            return false;
        }
            ...
            if (xhr.readyState === XMLHttpRequest.DONE) {
                if (xhr.status === 200) {
					...
					if(result.code==200)
					{
                        ...
                        abp.auth.setToken(result.item.accessToken, expireDate);
                        callback();   
					}
					else {
        ...
        xhr.send("{" + "usernameOrEmailAddress:'" + usernameOrEmailAddress + "'," + "password:'" + password + "'," + "secret:'" + secret + "'," + "platform:'PC'}");
    };
    ...
    abp.swagger.openAuthDialog = function (loginCallback) {
		...
		createInput(modalUxContent, 'secret', 'secret', 'secret');
